...
@RestApi(baseUrl: 'https://apis.data.go.kr/B551011/KorService1')
abstract class TourServiceApi {
  ...
  Future<TourServiceResponseWrapper<TourImageInfoDto>> getTourImageInfo(
      ...
      @Query('numOfRows') int numOfRows, // 한페이지결과수
  ...
  Future<TourServiceResponseWrapper<TourFestivalInfoDto>> getTourFestivalInfo(
      ...
      @Query('eventEndDate') String eventEndDate, // 행사종료일(형식 :YYYYMMDD)
      @Query('areaCode') String areaCode, // 지역코드(지역코드조회 참고)
      @Query('sigunguCode') String sigunguCode, // 시군구코드(지역코드조회 참고)
      @Query('modifiedtime') String modifiedtime, // 수정일(형식 :YYYYMMDD)
      );
